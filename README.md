...
- **중복 자료 스킵**: 추출 직후 `콘텐츠 해시`가 이미 처리된 항목과 일치하면 AI를
  호출하지 않고 원본을 가리키는 메모와 함께 `검토`로 라우팅합니다(비용·중복 방지).
- **병합 후보 자동 탐지**: 개념/엔터티 upsert 시 제목이 정확히 일치하면 그대로 재사용,
  별칭을 통해 매칭되면 기존 항목을 재사용하되 새 표기를 별칭으로 추가하고
  `검토 상태=병합 후보`로 표시해 사람이 확인하도록 합니다.
